...
//			for(int i=1; i<=monstersLength; i++) {
//				System.out.println("r: "+ monsters[i].p.r  + " c: " +monsters[i].p.c + " d: "+monsters[i].d);
//			}
//		
	...
	private static void movePackman(int turn) {
		...
		for(int d1=0; d1<4; d1++) {
			...
			for(int d2=0; d2<4; d2++) {
				...
				if(!next2.inRange() || visited.containsKey(next2)) continue;
				directions.add(next2);
				visited.put(next2, true);
				
				for(int d3=0; d3<4; d3++) {
					Point next3 = new Point(next2.r + dx[d3], next2.c + dy[d3]);
					if(!next3.inRange() || visited.containsKey(next3)) continue;
					...
					for(Point direction : directions) {
						for(int j=1; j<= monstersLength; j++) {
							if(isDead[j] == -1 && (monsters[j].p.r == direction.r && monsters[j].p.c == direction.c)) {
								cnt++;
							}
						}
					}
					...
					if(cnt > maxCnt) {
						//for(Point d: directions) System.out.println("@@@ r: "+ d.r + " c: "+d.c);
					...
					directions.remove(next3);
					visited.put(next1, false);
				}
				directions.remove(next2);
				visited.put(next1, false);
			}
			directions.remove(next1);
			visited.put(next1, false);
		...
		for(int i=1; i<= monstersLength ; i++) {
			...
			while(cnt < 8) { //8번 반
				...
				if(!next.inRange() || (next.r == packman.r && next.c == packman.c) || 
						isDead[i] != -1 && isDead[i] > turn) { //2에 죽음 isDead= 4 4>3  4>4
					if(cur.d == 7) cur.d = 0;
